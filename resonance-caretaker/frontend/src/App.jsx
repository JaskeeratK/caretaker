...
function App() {
  ...
  // Use your specific Codespace URL for the Backend
  const API_BASE_URL = 'https://jubilant-space-waffle-q79qq4wppxvg294jg-8000.app.github.dev';
  ...
  const fetchBurnout = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/burnout/score`);
      const data = await response.json();
      setBurnoutData(data);
    } catch (err) {
      console.error("Backend not reachable", err);
  ...
  const handleRelease = async () => {
    ...
    setIsReleasing(true);
    try {
      await fetch(`${API_BASE_URL}/journal/vent`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content: ventingText })
      });
